...
| UX-4 | Маркетплейс: поиск, выдача, избранное, фильтры | ✅ |
| UX-5 | Страница бизнеса: галерея, мастера, навигация | ✅ |
| UX-6 | Доверие и отзывы | ✅ |
| UX-7 | Доступность и надёжность | ✅ |

**Остаток:** фильтр по цене (нужен `min_price` в API) — по желанию.
...
- [x] **4.3 · Фильтры** — панель «Filtry» в `resultsBar`: язык обслуживания + удобства (клиентская
  фильтрация `sortedResults` по `languages`/`amenities` из payload), счётчик активных, «Wyczyść».
  Цена (`min_price` в API) — не сделано, отдельным пунктом при необходимости.
